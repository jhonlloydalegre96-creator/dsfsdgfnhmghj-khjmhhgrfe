...
function loadTransactions() {
  ...
  function isDuringWorkingHours(date) {
  ...
  transactions.forEach(t => {
    if (t.dueDate) {
      ...
      const isTodayDue = dueDate.toDateString() === today.toDateString();
      const diffMs = today - dueDate;

      if (diffMs > 0) {
        if (isTodayDue && isDuringWorkingHours(today)) {
          // During office hours, do not mark immediate overdue (grace until 5pm)
          t.status = "Borrowed";
        } else {
